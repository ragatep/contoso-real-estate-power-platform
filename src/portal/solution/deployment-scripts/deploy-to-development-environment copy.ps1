...
Write-Host @"
Download the most recent `ContosoRealEstateCustomControls.zip` solution into the folder '$tempReleaseFolder'. 
$publicRepo/releases?q=ContosoRealEstateCustomControls&expanded=true

Download the most recent `ContosoRealEstateCore.zip` solution into the folder '$tempReleaseFolder'. 
...
if (-not (Test-Path -Path "$tempReleaseFolder/ContosoRealEstateCustomControls.zip")) {
    Write-Host "The ContosoRealEstateCustomControls.zip file does not exist in the temp_releases folder." -ForegroundColor Red
    Write-Host @"
Download the most recent `ContosoRealEstateCustomControls.zip` solution into the folder . 
$publicRepo/releases?q=ContosoRealEstateCustomControls&expanded=true
"@ -ForegroundColor Yellow
    exit
}

# Check that the ContosoRealEstateCore.zip file exists
if (-not (Test-Path -Path "$tempReleaseFolder/ContosoRealEstateCore.zip")) {
    Write-Host "The ContosoRealEstateCore.zip file does not exist in the temp_releases folder." -ForegroundColor Red
    Write-Host @"
Download the most recent `ContosoRealEstateCore.zip` solution into the folder . 
$publicRepo/releases?q=ContosoRealEstateCore&expanded=true
"@ -ForegroundColor Yellow
    exit
}
...
Write-Host "Deploying 'ContosoRealEstateCustomControls.zip' to '$environmentName'" -ForegroundColor Green
pac solution import -p "$tempReleaseFolder/ContosoRealEstateCustomControls.zip" -a
if ($? -eq $false) {
    throw $_.Exception
    exit 1
}

Write-Host "Deploying 'ContosoRealEstateCore.zip' to '$environmentName'" -ForegroundColor Green
pac solution import -p "$tempReleaseFolder/ContosoRealEstateCore.zip" -a -ap --settings-file "$repoRoot/src/core/solution/deployment-scripts/temp_deploymentSettings_$azureEnv.json" 
if ($? -eq $false) {
    throw $_.Exception
    exit 1
}
