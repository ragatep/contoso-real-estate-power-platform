...
const ListingDetails: React.FC<ListingsProps> = () => {
    ...
    const getListingDetails = React.useCallback(async () => {
        ...
        if (listingId) {
            const listing = await ReservationApi.getListing(listingId);
    ...
    const [thumbsSwiper, setThumbsSwiper] = React.useState<SwiperClass | undefined>();

    const thumbOptions = React.useMemo(
        () => ({
            swiper: thumbsSwiper,
        }),
        [thumbsSwiper],
    ...
    const slideImages = React.useMemo(() => {
        return listingData?.images.map((image) => {
